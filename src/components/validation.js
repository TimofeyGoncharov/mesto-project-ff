...
function setSubmitAvailability(formElement, { inputSelector, submitButtonSelector, inactiveButtonClass }) {
    const inputElementsArr = Array.from(formElement.querySelectorAll(inputSelector));
    const targetElement = formElement.querySelector(submitButtonSelector);
    targetElement.classList.toggle(inactiveButtonClass, !inputElementsArr.every(isValid));
}

function hideInputError({formElement, inputElement, inputErrorClass, errorClass}) {
    const errorElement = formElement.querySelector(errorSelector(inputElement.id));
...
export function clearValidation( formElement, {inputSelector, inputErrorClass, errorClass, inactiveButtonClass, submitButtonSelector}) {
    const inputElementsArr = Array.from(formElement.querySelectorAll(inputSelector));
    inputElementsArr.forEach((inputElement) => {
        hideInputError({ formElement, inputElement, inputErrorClass, errorClass });
    });
    setSubmitAvailability(formElement, { inactiveButtonClass, inputSelector, submitButtonSelector });
}
